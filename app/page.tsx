...
import { featuredProperties, testimonials, cities } from '@/lib/data';
...
export default function Home() {
  ...
  return (
    <main className="bg-luxury-black">
      ...
      <section className="relative h-screen flex items-center overflow-hidden">
        ...
        <div className="absolute inset-0 z-0 flex slideshow-container">
          {heroImages.map((img, idx) => (
            <div key={idx} className="relative min-w-[100vw] h-full">
              <Image
                src={img}
        ...
        {/* Gradient Overlay */}
        <div className="absolute inset-0 z-5 bg-gradient-to-r from-luxury-black/80 via-transparent to-luxury-black/60"></div>
        ...
        {/* Content Panel */}
        <div ref={heroContent.elementRef} className={`relative z-20 ml-auto mr-8 md:mr-16 lg:mr-24 max-w-md opacity-0 ${heroContent.isVisible ? 'animate-fade-in-up' : ''}`} style={{animationFillMode: 'forwards'}}>
      ...
      <section className="py-32 px-8 md:px-16 bg-luxury-black">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div ref={aboutLeft.elementRef} className={`opacity-0 ${aboutLeft.isVisible ? 'animate-fade-in-up' : ''}`} style={{animationFillMode: 'forwards'}}>
            ...
            <div className={`opacity-0 ${aboutRight.isVisible ? 'animate-slide-in-left' : ''}`} style={{animationFillMode: 'forwards'}}>
              <div ref={aboutRight.elementRef} className="relative h-96 mb-8 overflow-hidden">
                <Image
      ...
      <section className="py-32 px-8 md:px-16 bg-luxury-charcoal border-y border-white/5">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-4 gap-16">
            <div ref={stats1.elementRef} className={`${stats1.isVisible ? 'animate-fade-in-up' : ''}`} style={{animationFillMode: 'forwards'}}>
            ...
            <div ref={stats4.elementRef} className={`${stats4.isVisible ? 'animate-fade-in-up' : ''}`} style={{animationFillMode: 'forwards'}}>
              ...
              <p className="text-gray-400 text-sm" style={{fontWeight: 300}}>
                <span className={`block ${stats4.isVisible ? 'animate-slide-in-left' : ''}`} style={{animationDelay: '0.2s', animationFillMode: 'forwards'}}>concierge services, meeting</span>
                <span className={`block ${stats4.isVisible ? 'animate-slide-in-left' : ''}`} style={{animationDelay: '0.4s', animationFillMode: 'forwards'}}>every need effortlessly.</span>
              </p>
            </div>
          </div>
      ...
      <section className="py-32 px-8 md:px-16 bg-luxury-black">
        <div className="max-w-7xl mx-auto">
          <div ref={featuredTitle.elementRef} className={`flex justify-between items-start mb-16 opacity-0 ${featuredTitle.isVisible ? 'animate-fade-in-up' : ''}`} style={{animationFillMode: 'forwards'}}>
            <div>
          ...
          <div className="md:hidden overflow-x-auto scrollbar-hide -mx-8 px-8 mb-12">
            <div className="flex gap-6" style={{width: 'max-content'}}>
              <div ref={property1.elementRef} className="w-[85vw] flex-shrink-0">
                <PropertyCard {...featuredProperties[0]} />
              </div>
              <div ref={property2.elementRef} className="w-[85vw] flex-shrink-0">
                <PropertyCard {...featuredProperties[1]} />
              </div>
              <div ref={property3.elementRef} className="w-[85vw] flex-shrink-0">
                <PropertyCard {...featuredProperties[2]} />
              </div>
            </div>
          </div>
          
          <div ref={featuredDesc.elementRef} className={`max-w-2xl opacity-0 ${featuredDesc.isVisible ? 'animate-fade-in-up' : ''}`} style={{animationFillMode: 'forwards'}}>
      ...
      <section className="py-32 px-8 md:px-16 bg-luxury-charcoal">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div ref={beliefsLeft.elementRef} className={`relative h-[600px] overflow-hidden opacity-0 ${beliefsLeft.isVisible ? 'animate-slide-in-left' : ''}`} style={{animationFillMode: 'forwards'}}>
              <Image
            ...
            <div ref={beliefsRight.elementRef} className={`opacity-0 ${beliefsRight.isVisible ? 'animate-fade-in-up' : ''}`} style={{animationFillMode: 'forwards'}}>
      ...
      {/* Contact CTA */}
      <section className="py-32 px-8 md:px-16 bg-luxury-black">
        <div ref={ctaSection.elementRef} className={`max-w-4xl mx-auto text-center opacity-0 ${ctaSection.isVisible ? 'animate-fade-in-up' : ''}`} style={{animationFillMode: 'forwards'}}>
          <h2 className="large-heading font-serif mb-8" style={{fontWeight: 300}}>
            DISCOVER YOUR<br/>
